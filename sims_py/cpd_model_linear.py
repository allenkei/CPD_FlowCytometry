...
class CPD(nn.Module):
    def __init__(self, args, half):
        ...
        self.linear_pi    = nn.Linear(self.d + self.p, self.K)
        self.linear_mean  = nn.Linear(self.d + self.p, self.K * self.D)
        self.linear_sigma = nn.Linear(self.d + self.p, self.K * self.D)
    ...
    def forward(self, x, z):
        ...
        pi = self.linear_pi(output).softmax(dim=1)
        mean = self.linear_mean(output).reshape(-1, self.K, self.D)
        sigma = F.softplus(self.linear_sigma(output)).reshape(-1, self.K, self.D) + 1e-6
